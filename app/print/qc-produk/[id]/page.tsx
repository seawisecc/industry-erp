...
export default async function PrintQcProdukPage({
  params,
}: {
  ...
  const [{ data }, { data: org }, { data: settings }] = await Promise.all([
    supabase
      ...
      .select(
        `id, no_batch_produksi, tanggal_produksi, qc_produk_jumlah_sampel,
         qc_produk_tanggal_uji, qc_produk_note, qc_produk_oleh, qc_produk_selesai,
         qc_produk_hasil,
  ...
  const hasil: Hasil[] = Array.isArray(b.qc_produk_hasil) ? b.qc_produk_hasil : [];
  const grup = new Map<string, Hasil[]>();
  for (const h of hasil) {
    const g = h.grup || "Lainnya";
    grup.set(g, [...(grup.get(g) || []), h]);
  }

  // Tanda tangan mengikuti pengaturan Lembar Pengujian QC
    ...
    <div className="min-h-screen py-4 sm:py-8 print:py-0">
      ...
      <div className="bg-white text-[#1a1a1a] a4-sheet max-w-[210mm] mx-auto shadow-xl print:shadow-none rounded-sm print:rounded-none p-[15mm] print:p-0 text-[12.5px] leading-relaxed">
        ...
        <div className="mt-5 flex items-start gap-4">
          ...
          <div>
            <span
              className={`inline-block px-3 py-1 text-[12.5px] font-bold border-2 ${
                b.qc_produk_selesai
                  ? "border-[#2F4D3A] text-[#2F4D3A]"
                  : "border-neutral-400 text-neutral-500"
              }`}
            >
              {b.qc_produk_selesai ? "PENGUJIAN SELESAI" : "DALAM PENGUJIAN"}
            </span>
            {b.qc_produk_note && (
              <div className="text-[11.5px] text-neutral-700 mt-2 max-w-[120mm]">
                Catatan: {b.qc_produk_note}
              </div>
            )}
            <div className="text-[11px] text-neutral-500 mt-1">
              Hasil uji ini menjadi dasar pelulusan batch oleh QA.
            </div>
          </div>
        </div>
        ...
        <div className="mt-10 pt-3 border-t border-neutral-300 text-[10px] text-neutral-400 flex justify-between">
          <span>Lembar Pengujian Produk Jadi — batch {b.no_batch_produksi}</span>
          <span>{org?.nama}</span>
        </div>
      </div>
    </div>
