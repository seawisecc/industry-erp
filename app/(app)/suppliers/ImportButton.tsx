...
const EXPECTED_COLUMNS = ["nama", "alamat", "nama_kontak", "no_telp", "email", "npwp"];

/** Deteksi delimiter dari baris header, biar gak salah baca kalau Excel export pakai ";" (umum di region Indonesia). */
...
export default function ImportButton() {
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (evt) => {
      ...
      const cleanText = text.replace(/^\uFEFF/, ""); // buang BOM kalau ada (dari Excel Mac)
      ...
      Papa.parse<Row>(cleanText, {
        ...
        complete: (result) => {
          ...
          const hasNamaColumn = headers.includes("nama");

          if (!hasNamaColumn) {
            setWarning(
              `Kolom "nama" tidak ditemukan. Kolom yang terbaca dari file: ${headers.join(", ") || "(tidak ada)"}. Pastikan baris pertama file berisi nama kolom yang benar.`
            );
          }

          const unknownCols = headers.filter((h) => h && !EXPECTED_COLUMNS.includes(h));
          if (unknownCols.length > 0 && hasNamaColumn) {
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-display text-lg font-semibold">Import Supplier dari CSV</h2>
            ...
            <p className="text-[12.5px] text-muted mb-3">
              Kolom yang dikenali: <b>nama</b> (wajib), alamat, nama_kontak, no_telp, email, npwp.
            </p>
                ...
                <table className="w-full text-[12px]">
                  <thead>
                    <tr className="bg-porcelain text-left">
                      ...
                      <th className="px-2 py-1.5">Kontak</th>
                      <th className="px-2 py-1.5">Telp</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rows.slice(0, 5).map((r, i) => (
                      <tr key={i} className="border-t border-line">
                        ...
                        <td className="px-2 py-1.5">{r.nama_kontak || "-"}</td>
                        <td className="px-2 py-1.5">{r.no_telp || "-"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            >
              {loading ? "Mengimport..." : `Import ${rows.length} Supplier`}
            </button>
          </div>
        </div>
      )}
