"use client";

/* ============================================================
   Buat revisi berikutnya dari formula ini.

   Dialognya sendiri (bukan ConfirmSave) karena butuh isian alasan.
   Alasan revisi wajib, dan itu bukan formalitas: silsilah formula
   yang tiap versinya cuma bernomor tidak menjawab pertanyaan yang
   selalu muncul belakangan, kenapa versi ini ada.
   ============================================================ */

import { useState } from "react";
import { createPortal } from "react-dom";
import { useRouter } from "next/navigation";
import { GitBranch, X } from "lucide-react";
import { reviseRndFormula } from "../actions";

export default function RevisiButton({
  id,
  noFormula,
}: {
  id: string;
  noFormula: string;
}) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [alasan, setAlasan] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  async function submit() {
    if (loading) return;
    if (!alasan.trim()) {
      setError("Isi alasan revisi dulu");
      return;
    }
    setLoading(true);
    setError("");
    try {
      const res = await reviseRndFormula(id, alasan.trim());
      if (res.ok) {
        setOpen(false);
        router.push(`/rnd/${res.id}`);
        router.refresh();
      } else {
        setError(res.error);
        setLoading(false);
      }
    } catch {
      setError(
        "Gagal, koneksi bermasalah atau aplikasi baru diperbarui. Muat ulang lalu coba lagi."
      );
      setLoading(false);
    }
  }

  return (
    <>
      <button
        onClick={() => setOpen(true)}
        className="inline-flex items-center gap-1.5 h-9 px-3.5 rounded-lg border border-line text-ink text-[12.5px] font-medium hover:bg-white/60 transition-colors"
      >
        <GitBranch size={15} /> Buat Revisi
      </button>

      {open &&
        createPortal(
          <div
            className="fixed inset-0 z-[100] flex items-center justify-center p-4"
            onClick={() => !loading && setOpen(false)}
          >
            <div className="absolute inset-0 bg-botanical-900/50 backdrop-blur-[2px]" />
            <div
              className="relative bg-[#FAF7F1] rounded-2xl shadow-2xl w-full max-w-md"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-start justify-between gap-3 px-5 pt-5 pb-3 border-b border-line">
                <div className="flex items-center gap-2.5">
                  <div className="rounded-lg p-2 bg-botanical-100 text-botanical-700">
                    <GitBranch size={18} />
                  </div>
                  <h3 className="font-display text-[16px] font-semibold text-ink">
                    Buat Revisi Formula
                  </h3>
                </div>
                <button
                  onClick={() => setOpen(false)}
                  className="text-muted hover:text-ink p-1 -mr-1"
                  aria-label="Tutup"
                >
                  <X size={18} />
                </button>
              </div>

              <div className="px-5 py-4">
                <p className="text-[12.5px] text-ink/75 leading-relaxed mb-3">
                  Seluruh isi <span className="font-mono">{noFormula}</span>{" "}
                  disalin ke versi baru bernomor turunan, dan versi ini tetap
                  tersimpan apa adanya. Hasil uji tidak ikut disalin, supaya
                  angka percobaan lama tidak terbawa ke percobaan berikutnya.
                </p>
                <label className="block text-[12px] font-medium text-muted mb-1.5">
                  Alasan revisi
                </label>
                <textarea
                  value={alasan}
                  onChange={(e) => setAlasan(e.target.value)}
                  rows={2}
                  placeholder="mis. tekstur terlalu encer, turunkan fase air"
                  className="w-full glass-input rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-botanical-700"
                />
                {error && (
                  <p className="text-clay-600 text-[12px] mt-2">{error}</p>
                )}
              </div>

              <div className="flex items-center gap-2 px-5 pb-5">
                <button
                  onClick={submit}
                  disabled={loading}
                  className="flex-1 inline-flex items-center justify-center gap-1.5 bg-botanical-700 text-white text-[13px] font-medium py-2.5 rounded-lg hover:bg-botanical-800 transition-colors disabled:opacity-60"
                >
                  <GitBranch size={15} />
                  {loading ? "Membuat..." : "Buat Revisi"}
                </button>
                <button
                  onClick={() => setOpen(false)}
                  disabled={loading}
                  className="px-4 py-2.5 rounded-lg border border-line text-[13px] font-medium text-muted hover:bg-white/60 transition-colors"
                >
                  Batal
                </button>
              </div>
            </div>
          </div>,
          document.body
        )}
    </>
  );
}
